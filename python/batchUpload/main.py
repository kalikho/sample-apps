...
def GetApprovedAgents():
    ...
    try:
        ...
        for response in stream:
            agentsList.append(response)
        return agentsList
    except grpc.RpcError as e:
        logging.error("gRPC error: %s", e)
#configure persistance and autoDir
def SetAgentConfig(persistance,autoDir,creds,agentDID):
    print(" ")
    print(f"== setting persistence and autodir for agent {agentDID}==")
    print(" ")
    try:
        ...
        print(f"persistence is set to True")
        print(f"autoDir is set to True")
        return True
...
if __name__ == '__main__':
    ...
    try:
        ...
        agentsList = GetApprovedAgents()
        print(" ")
        print(f"== found {len(agentsList)} registered agents ==")
        #upload files for each agent
        for agent in agentsList:
            res = SetAgentConfig(persist_on,autoDir_on,creds,agent.agent.agentIdentity)
            if res == False:
                print(f"== error while setting agent configuration for agent ",agent.agent.agentIdentity)
                exit()
            #Iterate over each file in the dir and upload
            files=list_files_in_directory(dirpath)
            print(" ")
            print(f"== file upload starts for agent {agent.agent.agentIdentity} ==")
            agentDID = agent.agent.agentIdentity
            for file_path, file_name in files:
                try:
                    ...
                    print(" ")
                    print(f"== upload completed for agent {agentDID} ==")
                except grpc.RpcError as e:
                    print(f"Upload failed for agent {agentDID} file {file_name}. gRPC error: {e.code()}: {e.details()}")
                    continue
    except grpc.RpcError as e:
